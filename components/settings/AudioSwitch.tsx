...
const AudioSwitch = connector(
	({ audioMuted = true, audioInitialized }: AudioSwitchProps) => {
		...
		const handleSize = () => {
			...
			canvas.style.width = `${Math.floor(
				containerRect.height * aspectRatio
			)}px`;
		};
		...
		return (
			<div
				...
				style={{
					...
					opacity: audioInitialized ? 1 : 0,
					transform: `translateY(${audioInitialized ? "0" : "-30vh"})`,
				}}
			>
				<div
